...
async function seedOrderItems() {
    const ordersList = await db.select({id: orders.id}).from(orders).where(and(gt(orders.id, 40000), lte(orders.id, 50000)));
    const productsList = await db.select({id: products.id}).from(products).where(and(gt(products.id, 0), lte(products.id, 6000)));
